span <- function(ra, declin, radii = 0.01, lim = 2, ovsm = 2, run = 5000, type = "LSP", rang = 2, ploterr = FALSE, alldb = FALSE, usenyq = TRUE, geo = FALSE, nonper = FALSE, quiet = FALSE) {
    ...
    for (i in 1:length(objects$entries)){
      ...
      if (nonper == FALSE){
        ...
        if (type == "LSP"){
          ...
          ans <- CNN(ts, filt = spurs, ofac = ovsm, from = freqmin, to = freqmax, nt = nt, b = b, pixsize = 28)
