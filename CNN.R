CNN <- function(x, filt, ofac = 1, from = 0.001, to = 20, nt = 150, b = 3, pixsize = 128, amp = 1, sam = 0.9, grad = TRUE) {
  ...
  for (k in 1:m) {
    ...
    pos <- weighted.mean(x[,2], (1 / x[,3])) + amp
    
    neg <- weighted.mean(x[,2], (1 / x[,3])) - amp
    ...
    write.table(imagm, file = paste("F:/Documents/PhD/matdump/", as.character(1/freq[k]), ".csv", sep=""), sep = ",", quote = FALSE, row.names = FALSE, col.names = FALSE)
    
    imagm <- as.vector(imagm)
    
    write.table(imagm, file = paste("F:/Documents/PhD/vecdump/", as.character(1/freq[k]), ".csv", sep=""), sep = ",", quote = FALSE, row.names = FALSE, col.names = FALSE)
