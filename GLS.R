GLS <- function(x, filt, ofac = 1, from = 0.001, to = 20, nt = 150, b = 3, plot = TRUE) {
  ...
  for (k in 1:m) {
    ...
    model <- lm(tsin[,2] ~ sin(2*pi*freq[k]*tsin[,1]) + cos(2*pi*freq[k]*tsin[,1]), data = as.data.frame(tsin[,1:2]), weights = 1/tsin[,3])
    
    coeff <- model$coefficients
    
    coeff[is.na(coeff)] <- 0
    
    yp <- (coeff[1] + coeff[2]*sin(2*pi*freq[k]*t) + coeff[3]*cos(2*pi*freq[k]*t))
    
    pwr[k] <- sum((y - yp)^2 / err^2)
  ...
  for (k in 1:length(redf)) {
    ...
    model <- lm(tsin[,2] ~ sin(2*pi*redf[k]*tsin[,1]) + cos(2*pi*redf[k]*tsin[,1]), data = as.data.frame(tsin[,1:2]), weights = 1/tsin[,3])
    
    coeff <- model$coefficients
    
    coeff[is.na(coeff)] <- 0
    
    yp <- (coeff[1] + coeff[2]*sin(2*pi*redf[k]*t) + coeff[3]*cos(2*pi*redf[k]*t))
    
    redn[k] <- sum((y - yp)^2 / err^2)
