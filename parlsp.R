parlsp <- function(x, times = NULL, from = NULL, to = NULL, type = c("frequency", "period"), ofac = 1, alpha = 0.01, plot = TRUE, ...) {    
  type <- match.arg(type)
  ...
  w <- 2 * pi * freq
  PN <- rep(0, n.out)
  PN <- foreach(i = 1:n.out, .combine = 'c') %dopar% {
    (sum(y * cos(w[i] * (t - 0.5 * atan2(sum(sin(w[i] * t)), sum(cos(w[i] * t)))/w[i]))))^2 / (sum(cos(w[i] * (t - 0.5 * atan2(sum(sin(w[i] * t)), sum(cos(w[i] * t)))/w[i])) * cos(w[i] * (t - 0.5 * atan2(sum(sin(w[i] * t)), sum(cos(w[i] * t)))/w[i])))) + (sum(y * sin(w[i] * (t - 0.5 * atan2(sum(sin(w[i] * t)), sum(cos(w[i] * t)))/w[i]))))^2 / (sum(sin(w[i] * (t - 0.5 * atan2(sum(sin(w[i] * t)), sum(cos(w[i] * t)))/w[i])) * sin(w[i] * (t - 0.5 * atan2(sum(sin(w[i] * t)), sum(cos(w[i] * t)))/w[i]))))
  }
