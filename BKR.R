BKR <- function(x, filt, ofac = 1, from = 0.001, to = 20, nt = 150, b = 3, plot = TRUE) {
  ...
  for (k in 1:m) {
    ...
    for (i in 1:length(t)){
      ...
      c[i] <- sum(xip1 <= x[ip1[i]] & xip <= x[ip[i]])
      
    }
    
    pwr[k] = sum((length(t)*c - r*r1)^2.0)
  ...
  if (plot == TRUE){
    ...
    plot(freq, pwr, pch=19, type="n", main="BKR", xlab = "Frequency (Cycles/day)", ylab = "BKR Statistic", ylim = c(min(pwr), max(pwr)))
    
    lines(freq, pwr, type="l")
    
  }
  
  sp.out <- list(scanned = freq, power = (max(pwr) - pwr), data = datanames, n = length(y), type = type, ofac = ofac, n.out = n, alpha = alpha, sig.level = level, 
                 peak = max(max(pwr) - pwr), peak.at = peak.at, p.value = p)
}