lccleans <- function(data, lim = 100, spur = FALSE, radii = 0.1, maxp = 20000, pero = 1.0, bins = 100, norm = TRUE, len = 200, seedno = 10) {
  ...
  for (i in 1:length(data$Name)){
    ...
    fts[,1] <- ((((fts[,1] - fts[,1][which.max(abs(fts[,2]))]) / (per[i])) + 0.25) %% 1)
    ...
    plot(fulfts[,1], fulfts[,2], main = paste("", data$Name[i], " (", data$Type[i], ") Folded Light Curve duplicated from 0.0 - 1.0 to -1.0 to 1.0", sep=""), xlab = paste("Phase at a period of ", per[i], " days", sep=""),
