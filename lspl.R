lspl <- function(x, times = NULL, from = NULL, to = NULL, type = c("frequency", "period"), ofac = 1, alpha = 0.01, plot = TRUE, ...) {    
  type <- match.arg(type)
  ...
  if (p.min > 1){
    ...
    freq <- (seq(p.min, p.max, by = 100*step))
    
  }
  else if (p.max < 1){
    
    freq <- (seq(fr.d/10, f.max/10, by = step))
    ...
    freq1 <- (seq(0.1, f.max/10, by = step))
    
    freq1 <- 1 / (freq1 * 10)
    
    freq1 <- sort(freq1)
    
    freq2 <- (seq(1, p.max, by = 100*step))
