  ...
  if (length(objects$entries) < 1){
    ...
    for (i in 1:length(objects$entries)){
      ...
      for (k in 1:redo){
        ...
        ans <- LCGEN(ts, fit = fit, melc = meanmag, amplc = amplitude, from = 1/ansset$permax[k], to = 1/ansset$permin[k], jit = jit, noimod = noimod, seedno = seedno, pop = pop, pairups = ps, nogen = 50, crossover = crov, mutation = 0.03, fdif = 0.6, dfrac = 0.7, clipcut = clipcut, bins = bins, tenper = tenper)
