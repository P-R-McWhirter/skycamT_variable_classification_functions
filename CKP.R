CKP <- function(x, IP, filt, ofac = 1, from = 0.001, to = 20, nt = 150, b = 3, plot = TRUE) {
  ...
  sigt <- 0.4 #This might be too low!
  
  print(paste("Starting trial period estimation. # of trials per bin: ", nt, " and # of bins: ", b, ".", sep=""))
  
  print(paste("Using values of sigy = ", sigy, " and sigt = ", sigt, "", sep=""))
  
  IP <- IP / (1 / (sqrt(2 * pi) * sigy))
  ...
  for (k in 1:b){
    ...
    #plot(Sj[,1], Sj[,(k+1)], pch=19, type="n", main="", xlab = "Frequency (1/days)", ylab = "Spectral Window", ylim = c(min(Sj[,(k+1)]), max(Sj[,(k+1)])))
    
    #lines(Sj[,1], Sj[,(k+1)], type="l")
  ...
  ham <- 0.54 + 0.46 * cos(pi * dift / ttot) 
  
  for (k in 1:m) {
    
    #print(k)
    
    Gt <- exp(-2.0 * sin(pi * abs(dift) * freq[k])^2 / sigt^2)
    
    pwr[k] <- (1 / n^2) * sum(sum((Gx - IP) * Gt * ham))
    
  }
  
  pwr <- (sqrt(n * sigt) / IP) * pwr
  ...
  for (k in 1:length(redf)) {
    ...
    Gt <- exp(-2.0 * sin(pi * abs(dift) * redf[k])^2 / sigt^2)
    
    redn[k] <- (1 / n^2) * sum(sum((Gx - IP) * Gt))
    
  }
  
  redn <- (sqrt(n * sigt) / IP) * redn
  ...
  if (plot == TRUE){
    ...
    plot(freq, pwr, pch=19, type="n", main="CKP", xlab = "Frequency (Cycles/day)", ylab = "Correntropy Kernelized Periodogram", ylim = c(min(pwr), max(pwr)))
...
CKP2 <- function(x, IP, ofac = 1, from = 0.001, to = 20, nt = 150, b = 3, plot = TRUE) {
