LCPEAN2 <- function(ra, declin, radii = 0.01, lim = 2, fit = "lsp", redo = 5, seedno = c(20, 30, 40, 50, 60), pop = 20, ps = 20, nogen = 100, crov = 0.65, mut = 0.03, fdif = 0.6, dfrac = 0.7, alldb = FALSE, geo = FALSE, quiet = FALSE) {
  ...
  if (length(objects$entries) < 1){
    ...
    featvec <- matrix(0, length(objects$entries), 4)
    ...
    if (geo == TRUE){
      ...
      geodist <- sqrt((declin - objects$DEClin)^2.0 + ((ra - objects$RA) / abs(cos(declin)))^2.0)
    ...
    for (i in 1:length(objects$entries)){
      ...
      for (k in 1:redo){
        ...
        ans <- LCGEN2(ts, fit = "lsp", melc = meanmag, amplc = amplitude, from = freqmin, to = freqmax, seedno = seedno[k], pop = pop, pairups = ps, nogen = nogen, crossover = crov, mutation = mut, fdif = 0.6, dfrac = 0.7)
        
        per[k] <- as.numeric(ans$period)
        
        chi2[k] <- as.numeric(ans$chi2)
      ...
      print(paste("Chi-squared minimized Period found: ", as.character(per[which.min(chi2)]), ".", sep=""))
      ...
      print(paste("Vuong Closeness Test maximized Period found: ", as.character(per[which.max(rowSums(LRT))]), ".", sep=""))
      ...
      features[3] <- per[which.min(chi2)]
      
      features[4] <- per[which.max(rowSums(LRT))]
    ...
    colnames(featvec) <- c("USnoref", "#.Observations", "Output.Periods", "Genetic.Period", "Vuong.Period")
