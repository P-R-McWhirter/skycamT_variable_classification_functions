lcxpl <- function(ra, declin, radii = 0.01, lim = 2, ovsm = 2, run = 5000, type = "LSP", rang = 2, ploterr = FALSE, alldb = FALSE, usenyq = TRUE, geo = FALSE, nonper = FALSE, trueper = FALSE, actper = 1.0, quiet = FALSE) {
    ...
    for (i in 1:length(objects$entries)){
      ...
      if (nonper == FALSE){
        ...
        if (trueper == FALSE){
          ...
          sper <- spurperlsp(ts, from = freqmin, to = freqmax, ofac = ovsm, name = objects$usnoref[i], plot = !quiet)
