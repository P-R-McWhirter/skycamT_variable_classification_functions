LCPE <- function(data, radii = 0.001, lim = 100, fit = "lsp", jit = 0, noimod = 1, sigk = 2.5, seascoe, trendrem = FALSE, rangcut = 0.2, gaufil = c(0.2, 10), redo = 3, seed = c(20, 30, 40), pop = 20, ps = 20, nogen = 100, crov = 0.65, mut = 0.03, fdif = 0.6, dfrac = 0.7, clipcut = 0.2, bins = 20, usenyq = FALSE, quiet = TRUE, fulltest = TRUE, tenper = FALSE) {
  ...
  for (i in 1:length(data$Name)){
    ...
    object <- LCPEAN(RA[i], DEC[i], radii, lim, fit = fit, jit = jit, noimod = noimod, sigk = sigk, seascoe = seascoe, trendrem = trendrem, rangcut = rangcut, gaufil = gaufil, redo = redo, seedno = seed, pop = pop, ps = ps, nogen = nogen, crov = crov, mut = mut, fdif = fdif, dfrac = dfrac, clipcut = clipcut, bins = bins, alldb = FALSE, geo = TRUE, quiet = quiet, fulltest = TRUE, tenper = tenper)
  ...
  colnames(result) <- c("Name", "Type", "Period", "USnoref", "#.Observations", "Output.Periods", "Fit.Stat", "Genetic.Period", "Vuong.Period", "Vuong.Confidence", "Vuong.Day.Confidence", "Num.Spurious", "Period.+1Alias", "Period.-1Alias", "Period.+0.5Alias", "Period.-0.5Alias")
