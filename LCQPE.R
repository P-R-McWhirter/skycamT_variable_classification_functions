LCQPE <- function(data, radii = 0.001, lim = 100, redo = 5, seed = c(20, 30, 40, 50, 60), pop = 20, ps = 20, nogen = 100, crov = 0.65, mut = 0.03, fdif = 0.6, dfrac = 0.7, clipcut = 0.2, bins = 20, usenyq = FALSE, quiet = TRUE, fulltest = TRUE) {
  ...
  for (i in 1:length(data$Name)){
    ...
    object <- LCPEAN(RA[i], DEC[i], radii, lim, fit = "bgls", redo = redo, seedno = seed, pop = pop, ps = ps, nogen = nogen, crov = crov, mut = mut, fdif = fdif, dfrac = dfrac, clipcut = clipcut, bins = bins, alldb = FALSE, geo = TRUE, quiet = quiet, fulltest = TRUE)
  ...
  colnames(result) <- c("Name", "Type", "Period", "USnoref", "#.Observations", "Output.Periods", "Genetic.Period", "Vuong.Period", "Vuong.Confidence", "Period.+1Alias", "Period.-1Alias", "Period.+0.5Alias", "Period.-0.5Alias")
