LCPEAN <- function(ra, declin, radii = 0.01, lim = 2, fit = "lsp", jit = 0, noimod = 1, sigk = 2.5, seascoe, trendrem = TRUE, rangcut = rangcut, gaufil = c(0.2, 10), redo = 5, seedno = c(20, 30, 40, 50, 60), pop = 20, ps = 20, nogen = 100, crov = 0.65, mut = 0.03, fdif = 0.6, dfrac = 0.7, clipcut = 0.2, bins = 20, alldb = FALSE, geo = FALSE, quiet = FALSE, fulltest = TRUE, tenper = FALSE) {
  ...
  if (length(objects$entries) < 1){
    ...
    featvec <- matrix(0, length(objects$entries), 8)
    ...
    for (i in 1:length(objects$entries)){
      ...
      spurs <- c(spurs*3, spurs*2, spurs, spurs/2, spurs/3, 1/29.53)
      ...
      if (length(ts[,1]) > 2000){
        ...
        for (k in 1:redo){
          ...
          ans <- LCGEN(ts, spurs = spurs, fit = fit, melc = meanmag, amplc = amplitude, from = freqmin, to = freqmax, jit = jit, noimod = noimod, gaufil = gaufil, seedno = seedno[k], pop = pop, pairups = ps, nogen = nogen, crossover = crov, mutation = mut, fdif = 0.6, dfrac = 0.7, clipcut = clipcut, bins = bins, tenper = tenper)
        ...
        print(paste("Object has less than 2000 data points, using a frequency spectrum for improved speed-up..."))
        
        ans <- BGLSgen(ts, spurs = spurs, gaufil = gaufil, plow = 0.05, phigh = 1/freqmin, ofac = 5, dt = NULL, lent = NULL, spur = F, jit = jit, plot = T)
        
        if (abs(max(ans$logp) - min(ans$logp)) == 0){
          
          stop("Insufficient White Noise Jitter")
          
        }
        
        vuongconf <- as.numeric(vuongtest(ans$model, ans$onemodel)$LRTstat)
        
        vuongday <- as.numeric(vuongtest(ans$model, ans$daymodel)$LRTstat)
        
        print(paste("BGLS Period found: ", 1/ans$f[which.max(ans$logp)], ".", sep=""))
        
        print(paste("Vuong Closeness Test LRT for this period against a constant model: ", as.character(vuongconf), sep=""))
        
        print(paste("Vuong Closeness Test LRT for this period against a one-day periodic model: ", as.character(vuongday), sep=""))
        
        features[2] <- as.character("")
        
        features[3] <- ans$norm
        
        features[4] <- 1/ans$f[which.max(ans$logp)]
      ...
      if (length(ts[,1]) <= 2000 & (as.numeric(features[5]) > 100 | abs(as.numeric(features[5])-1) < 2e-2)){
        
        print("A long or spurious period was detected from the frequency spectrum. Running genetic algorithm for improved accuracy...")
    ...
    colnames(featvec) <- c("USnoref", "#.Observations", "Output.Periods", "Fit.Stat", "Genetic.Period", "Vuong.Period", "Vuong.Confidence", "Vuong.Day.Confidence", "Num.Spurious")
