lcan_f <- function(ra, declin, radii = 0.01, lim = 2, ovsm = 2, run = 5000, type = "LSP", rang = 2, ploterr = FALSE, alldb = FALSE, usenyq = TRUE) {
  ...
  if (length(objects$entries) < 1){
    ...
    for (i in 1:length(objects$entries)){
      ...
      if (type == "LSP"){
        ...
        ans <- SLLK(ts[,1:2], from = freqmin, to = freqmax, ofac = ovsm, plot = FALSE)
        ...
        print("Fitting Correntropy Kernelised Periodogram...")
        
        ans <- CKP(ts, IP, from = freqmin, to = freqmax, plot = FALSE)
  ...
  colnames(featvec) <- c("USnoref", "# Observations", "Slope of Linear Trend", "#1 Freq", "#1 p-value", "#2 Freq",
                         ...
                         "Autocorrelation Length", "Slotted Autocorrelation Length", "Renyi's Quadratic Entropy", "Non-QSO", "Regression Flag")
